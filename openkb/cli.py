...
def _add_single_file_locked(
    ...
    from openkb.metadata import consume_inbound_metadata
    inbound_meta = consume_inbound_metadata(kb_dir, file_path.name)
    eff_date = inbound_meta.get("date")
    eff_tags = inbound_meta.get("tags")
    is_snapshot = inbound_meta.get("snapshot", False)
...
@click.option(
    "--mode",
    type=click.Choice(["update", "snapshot"], case_sensitive=False),
    default="update",
    help="Import mode: 'update' (in-place) or 'snapshot' (dated copy).",
)
@click.option(
    "--date",
    "file_date",
    default=None,
    metavar="YYYY-MM-DD",
    help="Document date (YYYY-MM-DD). Autogenerated if omitted.",
)
@click.option(
    "--tag",
    "tags",
    multiple=True,
    help="Metadata tags (e.g. --tag burndown --tag standup).",
)
@click.pass_context
@_with_kb_lock(exclusive=True)
def add(ctx, path, from_pageindex_cloud, mode="update", file_date=None, tags=None):
    ...
    if looks_like_url(path):
        ...
        fetched = fetch_url_to_raw(path, kb_dir, mode=mode, date=file_date, tags=tag_list)
        if fetched is None:
            return
    ...
    if target.is_dir():
        ...
        eff_date, eff_tags = autogenerate_metadata(target.name, file_date=file_date, tags=tag_list)
        raw_dir = kb_dir / "raw"
        raw_dir.mkdir(parents=True, exist_ok=True)
        if mode == "snapshot":
            snap_name = derive_snapshot_filename(target.name, eff_date, raw_dir)
            snap_dest = raw_dir / snap_name
            shutil.copy2(target, snap_dest)
            target_to_add = snap_dest
        else:
        ...
        add_single_file(target_to_add, kb_dir)
